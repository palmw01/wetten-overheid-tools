...
## Projectstructuur
- `wettenbank-mcp/`: TypeScript MCP-server die koppelt met wetten.overheid.nl.
- `mcp-debugger/`: Web-based interface om MCP-tools te testen en JSON-outputs te inspecteren.

## Development Richtlijnen
- **TypeScript:** Gebruik strikte typen. De schemas in `shared/schemas.ts` zijn de "source of truth".
- **MCP Protocol:** Volg de officiële MCP-specificaties voor tool-definities en responses.
- **JSON Focus:** De tools moeten pure, valide JSON retourneren die makkelijk te parsen is door AI-clients.
