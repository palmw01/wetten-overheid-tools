...
function registerTool(definition: Tool, handler: ToolHandler): void {
...
registerTool(
  {
    name: "system_check",
    description:
      "Geeft serverinformatie terug: timestamp, uptime, Node-versie en geheugengebruik. " +
...
async function connectMcpServer(serverPath: string): Promise<void> {
  const client = new Client(
    { name: "mcp-debugger-client", version: "1.0.0" },
    { capabilities: {} }
  );
  ...
  console.error(
    `${tools.length} tools geladen van MCP-server: ${serverPath}\n` +
